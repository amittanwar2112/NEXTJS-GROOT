...
class AutoSuggest extends Component {
  ...
  renderAutosuggest(suggestions) {
    ...
    return (
      <div
        className={`${styles.searchWidgetDropdown} ${trains ? styles.trainsSuggestionBox : ''}`}
        id="autosuggest">
        <ul className={`${styles.dropdownList}`}>
          {suggestions.map((suggestion, index) => {
            ...
            if (isHeading) {
              return (
                <p className="grey75 ico14 padB10 padLR20" key={index}>
                  {suggestion.content}
                </p>
              );
            } else {
              return (
                <li
                  key={index}
                  onClick={(e) => {
                    suggestionClicked(e, suggestion, index);
                  }}>
  ...
  render() {
    ...
    const comp = (
      <div className={`${styles.flex} ${styles.row}`} onClick={this.handleClick}>
        <span>
          <i className={`${styles.iconsearch} ico15 greyDr`}></i>
        </span>
        <AutoSuggestInput
    ...
    return (
      <OutsideAlerter cb={this.props.outSideClick} inlineStyle={inputStyle}>
        {comp}
      </OutsideAlerter>
    );
  }
