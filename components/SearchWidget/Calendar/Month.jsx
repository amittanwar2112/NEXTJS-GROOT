...
const Month = ({ month, year, getDisabledDays = ()=>{}, updateDate, selectedDay, renderDate, daysOfRun = {} }) => {
  ...
  return(
    <div className="DayPicker-Month">
      <div className="DayPicker-Caption" role="heading">{getFormattedDate(firstDate, 'mmmm yyyy')}</div>
      <div className="DayPicker-Weekdays">
        <div className="DayPicker-WeekdaysRow">
          {
            dayList.map((item,index) => (
              <li key={index} className={"DayPicker-Weekday db"} role="columnheader"><span title={item}>{item}</span></li>
            ))
          }
        </div>
      </div>
            ...
            <div className="DayPicker-Week" key={rIndex}>
              {
                new Array(colLength).fill(1).map((__, cIndex) => {
                  ...
                  if (today.getDate() === dateVal && today.getMonth() === month && today.getFullYear() === year) {
