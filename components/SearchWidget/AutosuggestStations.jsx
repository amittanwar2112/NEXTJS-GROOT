...
class AutoSuggestStations extends Component {
    ...
    return (
      <Fragment>
        <span className="padR15">
          <i className={`ico20 ${styles.icontrainsnew} grey75 `}></i>
        </span>
        <div>
          <p className="ico16 black padB5">{cn}</p>
          <p className="grey50 ico12">
            {suggestion.irctc_code} - {$stationName}
          </p>
        </div>
      </Fragment>
    );
  ...
  getRecentAndPopularList() {
    ...
    if (prevSearchCache.length > 0) {
      ...
      recentAndPopularList = recentAndPopularList.concat(
        findRecentlySearchedUniqueStations(prevSearchCache)
      );
    }
