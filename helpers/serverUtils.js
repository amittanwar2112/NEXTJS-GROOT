export function customTemplatePlaceholder({ type, text, subtext = '' }) {
  ...
  templateString +=
    'style="z-index: 20;overflow-y: hidden;max-height: 100px;transition: all .2s cubic-bezier(0, 1, 1, 1);position: absolute;bottom: 15%;left: 5%;width: 90%;float: left;background: rgba(0,0,0,0.5);padding: .938em;-webkit-border-radius: 25px;font-weight: 500;font-style: normal;text-align: center;';
  if (type === 'detectNetworkOfflineDiv') {
    templateString += ' display: none ';
  }
  templateString += '">';
  templateString +=
    '<span style="line-height: 1.7; font-size: .875rem; color: #fff; width: 100%; float: left;">';
  templateString += text;
    ...
    function detectNetworkHandler(event) {
      ...
      if (navigator.onLine) {
        ...
        if (!detectNetworkOfflineDiv) {
          ...
          newDiv.innerHTML = '${customTemplatePlaceholder({
            type: 'detectNetworkOfflineDiv',
            text: 'Oops! Seems like you went offline',
            subtext: 'This message will hide itself once you are back online'
          })}';
