...
async function generateFaqTemplate(req, path, metaData) {
  ...
  if (metaData) {
    ...
    pageInterlinks =
      pathMetaMap[path] && pathMetaMap[path].interlinks ? pathMetaMap[path].interlinks : [];
    ...
    if (pathMetaMap[path] && data) {
      ...
      const template = `<section class="trainsfaqouterWrap  style="max-width:1200px" itemscope="" itemtype="https://schema.org/FAQPage">
        <div class="width100 ">
          ${content ? `<div class="contentWrap marginB15">${marked(content)}</div>` : ``}
          ...
          ${faq_content
            .map((faqItem, index) => {
              ...
              return `<div class="tab" itemscope="" itemprop="mainEntity" itemtype="https://schema.org/Question">
...
function storeInRedis(templateWithId, key) {
  if (global.__REDISCONFIG__.REDISINSTANCE) {
    ...
    zlib.gzip(templateWithId, { encoding: null }, function (compressErr, compressedTemplate) {
      ...
      // Redis Storing
      global.__REDISCONFIG__.REDISINSTANCE.set(
        key,
        compressedTemplate,
        'EX',
        604800,
...
async function sendNewTemplate(req, res, isMobile, shouldUpdateCache = false, key) {
  try {
    ...
    const metaData =
      (await getFAQMetaDataCall(`${process.env.PAGE_ROVER}/gt_rail/api/v1/get_seo_page_data`)) ||
      {};
...
function generatePopularTemplate(linksData) {
  ...
  if (title && links && links.length > 0) {
    return `<div class="width100  padT10 padB20 padL5">
                <p class="ico16 fmed blueLight">${marked(title)}</p>
            </div>
            <div class="padB15 popularWrap">
            ${links
              .map((item) => {
                return `<div class="pad0  marginB10">
...
function generateInterlinksTemplate(interlinks) {
  if (interlinks && interlinks.length > 0) {
    ...
    return `<section class="popularSearchWrap">
  ...
  try {
    if (USE_REDIS_CACHE_SEO && !cb && !hcb && global.__REDISCONFIG__.REDISINSTANCE) {
      return new Promise((resolve, reject) => {
        global.__REDISCONFIG__.REDISINSTANCE.get(
          new Buffer(key),
          function (err, compressedTemplate) {
