...
class OffersCarousel {
  ...
  getOfferURL(vertical, userid = '') {
    return `https://voyager.goibibo.com/api/v1/pagemaker/get_chunk_offers_data/?chunk=home_carousel&vertical=${vertical}&flavour=v3${
  ...
  handleArrowIcon() {
    ...
    if (
      this.perPage === this.slotData.length ||
      this.carousel.currentSlide + 1 === this.slotData.length
    ) {
        ...
        <div class="fl posRel" style="${this.perPage !== 3 ? 'width: 100%;' : 'maxWidth: 100%;'}">
          <div class="siema fl" style="${carouselWrap}">
            ${slotData.map((slotInfo, index) => {
              ...
              return `<div class="offerBlkWdgtNew posRel curPoint" onclick="window.open('${url}', '_blank', 'noopener')" style="${offerBlkWidget}">
                    <img id="offer_carousel_img_${index}"src="${
                showImg ? img_url : ''
              }" data-src="${!showImg ? img_url : ''}" alt="${
                'Offers carousel image ' + index
              }" style="${carouselImg}" />
          ...
          <div id="_carousel_next_btn" class="curPoint posAbs fl" style="${rightArrWrap}left: ${
      this.nextArwIconPos
    }">
            <div class="circleBlc fl" style="${arrowBg}">
