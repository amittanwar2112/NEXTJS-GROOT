...
const SearchWidget = (props) => {
  ...
  const beginSearch = () => {
    ...
    if (hasNoError) {
      let [emptyString1, trainsText, srpText, oldSrc, oldDest, oldDate, quota, emptyString2] =
  ...
  const validateEntries = () => {
    ...
    if (
      ...
      source.irctc_code === destination.irctc_code
    ) {
  ...
  const swapStations = () => {
    ...
    let destValue = Object.keys(destination).length
      ? `${destination.irctc_code}, ${destination.dn}`
      : '';
    ...
    <section className={styles.homeSearchWidget}>
      <aside className={`${styles.widgetRow} ${styles.borderBottom0}`}>
        <AutoSuggestStations
          ...
          labelClassName={`width45 posRel ${styles.borderBottomGrey}`}
          autoSuggestClassName={`${styles.textInput} width100 ${styles.ellipsis}`}
          isSrc
          inputStyle="margin-left:4px"
        />
        <span onClick={swapStations}>
          <i className={`${styles.iconswap3} goBlue ico30`}></i>
        </span>
        <AutoSuggestStations
          ref={destRef}
          trainRef={trainRef}
          dispatchToStore={dispatchToStore}
          resetErrorMessage={resetErrorMessage}
          labelClassName={`width45 txtRight ${styles.toDestination} ${styles.borderBottomGrey}`}
          autoSuggestClassName={`${styles.textInput} width100 ${styles.ellipsis} txtRight`}
          inputStyle="margin-left:1px"
        />
      </aside>
      <aside className={`padT15 ${styles.widgetRow}`}>
        <AutoSuggestTrains
          ...
          clickOnDisableStateHandler={validateEntries}
          ref={trainRef}
          dispatchToStore={dispatchToStore}
          resetErrorMessage={resetErrorMessage}
          labelClassName={`width100`}
          autoSuggestClassName={`${styles.textInput} width100`}
          inputStyle="margin-left:4px"
        />
      </aside>
      <aside className={`padT15 ${styles.widgetRow}`}>
      ...
      {errorMsg ? (
        <div className="red ico11 padT15 dib" style={{ textAlign: 'center' }}>
          {errorMsg}
        </div>
      ) : null}
    </section>
