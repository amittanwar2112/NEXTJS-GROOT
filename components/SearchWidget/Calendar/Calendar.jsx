...
const CalendarMain = ({ resetErrorMessage = () => {} }) => {
  ...
  const handleLocalStorageCache = () => {
    ...
    if (date) {
      const localSelectedDay = new Date(
        date.substr(0, 4),
  ...
  const handleAvlChange = () => {
    ...
    if (isActive && !availability.length && trainNumber) {
      ...
      callAvailability({ src, dest, trainNumber, classType: travelClass, month: monthApi }).then(
        (availability) => {
          dispatchToStore({
            availability
          });
  ...
  const renderDate = (dateObj, addedClass = '', isDisabled, handleDateClick) => {
    ...
    const checkForStyle = showAvl
      ? availability.length &&
        availability[dateVal - 1] &&
        (availability[dateVal - 1].status_color || 'lightgrey')
      : '';
    const style = checkForStyle
      ? `color: ${availability[dateVal - 1].status_color}; justify-content: center`
      : '';
    const bgStyle = checkForStyle
      ? checkForStyle !== 'lightgrey'
        ? `background-color: ${availability[dateVal - 1].status_color}; color: white;`
    ...
    const updateDate = () => {
      ...
      if (
        checkForStyle &&
        checkForStyle !== 'rgb(255, 21, 21)' &&
        checkForStyle !== '#ff1515' &&
        checkForStyle !== 'lightgrey'
      ) {
    ...
    return (
      <div className={`${styles.DayPickerDaywrap} ${styles.curPoint}`}>
        <div
          className={`${styles.DayPickerDay} ${styles.txtCenter} ${styles.addedClass} ${styles.availabilityStatus}`}
          style={{ bgStyle } + isDisabled ? { pointerEvents: 'none' } : ''}
          onClick={() => {
            handleDateClick({ isDisabled, dateVal });
            updateDate();
          }}>
          {dateVal}
        </div>
        {(showAvl &&
          ...
          availability[dateVal - 1].status && (
            <div
              className={`${styles.width100} ${styles.ico12} ${styles.availabilityStatus} ${styles.bold} ${styles.padT5} ${styles.flex}`}
              style={style}>
              {availability[dateVal - 1].status}
            </div>
          )) ||
    ...
    return (
      <div className={`${styles.padT20} ${styles.padLR20}`}>
        <div className={`${styles.trainTypeTabs}`}>
          <p
            className={`${styles.curPoint} ${isActive ? styles.active : ''}`}
            onClick={handleTrainNameClick}>
            <span>{TrainText}</span>
          </p>
          <p
            className={`${styles.curPoint} ${!isActive ? styles.active : ''}`}
            onClick={handleAllTrainClick}>
          ...
          <Fragment>
            ...
            <div className={`${styles.trainClassTabs}`}>
              {classes.map((class_type, index) => (
                <p
                  key={index}
                  className={`${styles.curPoint} ${
                    travelClass === class_type ? styles.active : ''
                  }`}
                  onClick={() => onChangeClassType(class_type)}>
        ...
        {isActive ? (
          <div className={`${styles.calendarFooter}`}>
            ...
            <div className={`${styles.padR10}`}>
              ...
              <span className={`${styles.ico12} ${styles.grey75}`}>
                Regret, Departed, Train doesnot run
              </span>
            </div>
          </div>
        ) : null}
    ...
    <Fragment>
      <div className={`${styles.width50}`}>
        <label>
          ...
          <p
            className={`${styles.widgetInput} ${styles.width100} ${styles.textInput} ${styles.width100}`}
            style={{ marginLeft: '1px' }}
            onClick={() => {
              console.log({ showCalendar });
              dispatchToStore({ showCalendar: true });
            }}>
            {day}, {dateInputValue}
          </p>
        </label>
          ...
          <label className={`${styles.padR20}`}>
            <span className={`${styles.padR5}`}>
              <input
                id="today"
                checked={today}
                onChange={(ev) => handleDateTabClick(ev, 0)}
                type="radio"
                name="day"
              />
            </span>
          ...
          <label className={`${styles.padL20}`}>
            <span className={`${styles.padR5}`}>
              <input
                id="today"
                checked={tomorrow}
                onChange={(ev) => handleDateTabClick(ev, 1)}
                type="radio"
                name="day"
              />
            </span>
