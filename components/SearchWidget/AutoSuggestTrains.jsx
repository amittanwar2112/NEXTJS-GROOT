...
class AutoSuggestTrains extends Component {
  ...
  listItemMarkup(suggestion) {
    ...
    if (
      suggestion &&
      suggestion.TrainText &&
      suggestion.TrainText.toLowerCase().startsWith(typedTxt)
    ) {
    ...
    return (
      <Fragment>
        <span className="padR15">
          <i className={`${styles.icontrainsnew}  ico20 grey75`}></i>
        </span>
        <div title={suggestion.dn}>
          <p className="ico16 black padB5">{$stationName}</p>
          <p className="grey50 ico12 lh12">
            {suggestion.source} - {suggestion.destination}
          </p>
        </div>
      </Fragment>
    );
  ...
  suggestionClicked(event, suggestion, index) {
    ...
    callAvailability({ src, dest, trainNumber, month }).then((availability) => {
      this.props.dispatchToStore({
        availability: availability
      });
    });
    const classTypeToBeUpdated =
      suggestion.classes && suggestion.classes.length ? suggestion.classes[0] : 'SL';
    this.props.dispatchToStore({
      train: suggestion,
  ...
  render() {
    ...
    return (
      <label className={labelClassName}>
        ...
        <AutoSuggest
          autoSuggestClassName={autoSuggestClassName}
          ref={this.autoSuggestRef}
          callback={this.onNewSearchQuery}
          suggestions={suggestions}
          listItemMarkup={this.listItemMarkup}
          ...
          showRecentsAndPopular={showRecentsAndPopular}
          closeClicked={this.closeClicked}
          setInputId={setInputId}
          trains={true}
          handleDisableState={disabled}
          clickOnDisableStateHandler={clickOnDisableStateHandler}
