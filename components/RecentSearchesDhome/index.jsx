    ...
    <section className="marginTB10">
					...
					<div className={`${styles.smallCardsWrap}`}>
          {
            recentTrainSearches.map((searchItem, index) => {
              ...
              return (
                <aside onClick={handleClick.bind(null, searchItem)} className={`${styles.recentSearchCard} ${styles.curPoint}`}  key={index}>
                  <div className={`${styles.cardHdr}`} >
                    <div className={`${styles.hdrTag}`} >
                      <span className="padR5"><i  className={`${styles.icontrainsnew} goBlue ico16`} ></i></span>
                      <span className={`${styles.hdrTagTxt}`} >{start + ' to ' + end}</span>
                    </div>
                    <span>
                      <span className="pad10"><i className={`${styles.iconarrowright} goBlue ico10`} ></i></span>
                    </span>
                  </div>
